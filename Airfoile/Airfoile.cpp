...
void Node::setPoint(double x, double y) {
    this->x = x;
    this->y = y;
}
double Node::getX() {
...
int Node::getInout() { // 재 : inout 추가로 인한 함수 추가2017_02_05
    return inout;
}
bool Node::operator==(Node* n) { // 재 : node와 node의 비교 2017-01-12
    if (x == n->getX()) {
        if (y == n->getY()) {
            if (id == n->getID())
                return true;
...
Face::Face(Node *sNode, Node *eNode) {
    centerNode = NULL;
...
void Face::setAttribute(int attribute) {
    this->attribute = attribute;
}
void Face::setStartCell(CellShape *cell) { // 재 : Cell => CellShape 2017-01-12
    this->sCell = cell;
}
void Face::setEndCell(CellShape *cell) { // 재 : Cell => CellShape 2017-01-12
    this->eCell = cell;
}
void Face::setStartNode(Node *node) {
    this->sNode = node;
}
void Face::setEndNode(Node *node) {
    this->eNode = node;
}
void Face::setCenterNode(Node *node) {
    centerNode = node;
}
void Face::setChilds(Face *c1, Face *c2) {
    Child1 = c1;
    Child2 = c2;
...
void Face::setDepth(int depth) {
    this->depth = depth;
}
void Face::setPrintID(int newID) {
    this->printID = newID;
}
void Face::setMyPrintID(int newID) {
    myPrintID = newID;
}
void Face::setParent(Face *parent) {
    curParent = parent;
}
int Face::getMyPrintID() {
    return myPrintID;
}
int Face::getMaxDepth() {
    return maxDepth;
}
int Face::getPrintID() {
    return this->printID;
...
int Face::getAttribute() {
    return attribute;
}
int Face::getCount() {
    return faceCount;
}
Node* Face::getStartNode() {
    return sNode;
}
Node* Face::getEndNode() {
    return eNode;
}
Node* Face::getCenterNode() {
    return centerNode;
}
CellShape* Face::getStartCell() { // 재 : CellShape로 반환값 변경  2017-01-12
    return sCell;
}
CellShape* Face::getEndCell() { // 재 : CellShape로 반환값 변경  2017-01-12
    return eCell;
}
Face* Face::getChild1() {
    return Child1;
...
bool Face::operator==(Face& f) { // 재 : face와 face의 일치 비교 2017-01-12
...
void Cell::setChildNodes(Node *L, Node *D, Node *R, Node *U, Node *C, int partialtype) { // 진: tail처리_수정 0307
...
void Cell::setNewFaces(Face *from, Face *child1, Face *child2, Node *Start, Node *Center, Node *End) {
    if (!from->isParent()) {
...
void Cell::setupMininodePlace(double x, double y) { // 재 : dotpoint함수가 너무 길어지는 것 같아서 분리시켰습니다. 2017-01-12
...
void Cell::setiscut(bool cut) {
    iscut = cut;
}
int Cell::getNodeCount() {
    return LUNode->getCount();
}
int Cell::getCellCount() {
    return cellCount;
}
int Cell::getFaceCount() {
    return LFace->getCount();
}
int Cell::getID() {//진delete
    if (id>0)return id;
    return NULL;
}
Node* Cell::getLUNode() {
...
void Cell::setDepth(int newDepth) {
    depth = newDepth;
}
void Cell::setID(int newID) {
    this->id = newID;
}
void Cell::setLUNode(Node *node) {
    LUNode = node;
}
void Cell::setLDNode(Node *node) {
    LDNode = node;
}
void Cell::setRDNode(Node *node) {
    RDNode = node;
}
void Cell::setRUNode(Node *node) {
    RUNode = node;
}
...
void Cell::dotPoint(double x, double y, int layer, int depth) { // 재 : LUCell, LDCell, RUCell, RDCell 모두 CellShape로 변환, Cell의 멤버함수에 접근 필요시 ((Cell*) __) 조치 2017-01-12
...
void Cell::confirmAll() { // 재 : Face의 eCell과 sCell을 CellShape로 변경해서 이 부분도 수정이 필요해 수정했습니다.  2017-01-12
...
bool Cell::checkNode(Node* n) { // 재 : Node를 인자로 주면 Cell이 인자를 가지고 있는지 확인하는 함수 2017-01-12
...
bool Cell::checkFace(Face* f) { // 재 : Face를 인자로 주면 Cell이 인자를 가지고 있는지 확인하는 함수 2017-01-12
    if (LFace == f || UFace == f || RFace == f || DFace == f)
        return true;
    return false;
}

void Cell::splitCell(int layer, int depth) { // 재 : Cell split 구현, depth 추가 2017-01-19
    const int OUTFLOW = 0, INLET = 1, TOP = 2, BOTTOM = 3, INTERRIOR = 4;
    if (LUCell == NULL) {    // 아직 이 Cell이 쪼개지지 않았으면, 쪼개라!
        //( 쪼개졌으면 LUCell, LDCell, RDCell, RUCell이 NULL이 아닐거니까 그중 하나만 체크한거임 )
        // 내부 Cell(Default)를 먼저 생성해둔다.
...
CellTriangle::CellTriangle(Node *HO, Node *VO, Node *WO) {
...
int CellTriangle::getTriCount() {
    return triCount;
}

Rectangle::Rectangle(double LDX, double LDY, double Cellsize, int WidthCount, int HeightCount) {//진_initcellsize설정
    LDx = LDX;    LDy = LDY;
...
int Rectangle::getNodeCount() {
    return initCell[0][0].getLDNode()->getCount();
}
int Rectangle::getCellCount() {
    return initCell[0][0].getCellCount();
}
int Rectangle::getFaceCount() {
...
void Rectangle::dotPoint(double x, double y, int layer = 0, int depth = 0) { //진 : 경계선노드처리 수정
